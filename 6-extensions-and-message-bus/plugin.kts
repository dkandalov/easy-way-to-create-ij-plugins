...
val consoleInputFilterProvider = ConsoleInputFilterProvider {
    arrayOf(InputFilter { text, _ ->
        show(text)
        if (text.contains("Contents/Home/bin/java")) emptyList()
        else null
    })
}
ApplicationManager.getApplication()
    .extensionArea.getExtensionPoint(ConsoleInputFilterProvider.INPUT_FILTER_PROVIDERS)
    .registerExtension(consoleInputFilterProvider, pluginDisposable)
